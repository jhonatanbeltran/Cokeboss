...
class cliente extends DbTable
{
	...
	public function __construct()
	{
		...
		// documento_cliente
		$this->documento_cliente = new DbField('cliente', 'cliente', 'x_documento_cliente', 'documento_cliente', '`documento_cliente`', '`documento_cliente`', 200, -1, FALSE, '`documento_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->documento_cliente->IsPrimaryKey = TRUE; // Primary key field
		$this->documento_cliente->Nullable = FALSE; // NOT NULL field
		$this->documento_cliente->Required = TRUE; // Required field
		$this->documento_cliente->Sortable = TRUE; // Allow sort
		$this->fields['documento_cliente'] = &$this->documento_cliente;

		// nombre_cliente
		$this->nombre_cliente = new DbField('cliente', 'cliente', 'x_nombre_cliente', 'nombre_cliente', '`nombre_cliente`', '`nombre_cliente`', 200, -1, FALSE, '`nombre_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombre_cliente->Sortable = TRUE; // Allow sort
		$this->fields['nombre_cliente'] = &$this->nombre_cliente;

		// apellido_cliente
		$this->apellido_cliente = new DbField('cliente', 'cliente', 'x_apellido_cliente', 'apellido_cliente', '`apellido_cliente`', '`apellido_cliente`', 200, -1, FALSE, '`apellido_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->apellido_cliente->Sortable = TRUE; // Allow sort
		$this->fields['apellido_cliente'] = &$this->apellido_cliente;

		// direccion_cliente
		$this->direccion_cliente = new DbField('cliente', 'cliente', 'x_direccion_cliente', 'direccion_cliente', '`direccion_cliente`', '`direccion_cliente`', 200, -1, FALSE, '`direccion_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->direccion_cliente->Sortable = TRUE; // Allow sort
		$this->fields['direccion_cliente'] = &$this->direccion_cliente;

		// telefono_cliente
		$this->telefono_cliente = new DbField('cliente', 'cliente', 'x_telefono_cliente', 'telefono_cliente', '`telefono_cliente`', '`telefono_cliente`', 200, -1, FALSE, '`telefono_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->telefono_cliente->Sortable = TRUE; // Allow sort
		$this->fields['telefono_cliente'] = &$this->telefono_cliente;

		// id_tipo_cliente
		$this->id_tipo_cliente = new DbField('cliente', 'cliente', 'x_id_tipo_cliente', 'id_tipo_cliente', '`id_tipo_cliente`', '`id_tipo_cliente`', 200, -1, FALSE, '`id_tipo_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_tipo_cliente->Sortable = TRUE; // Allow sort
		$this->fields['id_tipo_cliente'] = &$this->id_tipo_cliente;

		// email
		$this->_email = new DbField('cliente', 'cliente', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('documento_cliente', $rs))
				AddFilter($where, QuotedName('documento_cliente', $this->Dbid) . '=' . QuotedValue($rs['documento_cliente'], $this->documento_cliente->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->apellido_cliente->DbValue = $row['apellido_cliente'];
	...
	{
		return "`documento_cliente` = '@documento_cliente@'";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('documento_cliente', $row) ? $row['documento_cliente'] : NULL) : $this->documento_cliente->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@documento_cliente@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("clienteview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		$json = "";
		$json .= "documento_cliente:" . JsonEncode($this->documento_cliente->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->documento_cliente->CurrentValue != NULL) {
			$url .= "documento_cliente=" . urlencode($this->documento_cliente->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("documento_cliente") !== NULL)
				$arKeys[] = Param("documento_cliente");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->documento_cliente->CurrentValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->apellido_cliente->setDbValue($rs->fields('apellido_cliente'));
	...
	public function renderEditRow()
	{
		...
		$this->apellido_cliente->EditValue = $this->apellido_cliente->CurrentValue;
