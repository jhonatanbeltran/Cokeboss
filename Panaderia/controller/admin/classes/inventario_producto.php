...
/**
 * Table class for inventario_producto
 */
class inventario_producto extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'inventario_producto';
		$this->TableName = 'inventario_producto';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`inventario_producto`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_producto
		$this->id_producto = new DbField('inventario_producto', 'inventario_producto', 'x_id_producto', 'id_producto', '`id_producto`', '`id_producto`', 200, -1, FALSE, '`id_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id_inventario
		$this->id_inventario = new DbField('inventario_producto', 'inventario_producto', 'x_id_inventario', 'id_inventario', '`id_inventario`', '`id_inventario`', 200, -1, FALSE, '`id_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_inventario->IsPrimaryKey = TRUE; // Primary key field
		$this->id_inventario->Nullable = FALSE; // NOT NULL field
		$this->id_inventario->Required = TRUE; // Required field
		$this->id_inventario->Sortable = TRUE; // Allow sort
		$this->fields['id_inventario'] = &$this->id_inventario;

		// fecha_inventario
		$this->fecha_inventario = new DbField('inventario_producto', 'inventario_producto', 'x_fecha_inventario', 'fecha_inventario', '`fecha_inventario`', CastDateFieldForLike('`fecha_inventario`', 0, "DB"), 133, 0, FALSE, '`fecha_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fecha_inventario->IsPrimaryKey = TRUE; // Primary key field
		$this->fecha_inventario->Nullable = FALSE; // NOT NULL field
		$this->fecha_inventario->Required = TRUE; // Required field
		...
		$this->cantidad_inv_producto = new DbField('inventario_producto', 'inventario_producto', 'x_cantidad_inv_producto', 'cantidad_inv_producto', '`cantidad_inv_producto`', '`cantidad_inv_producto`', 3, -1, FALSE, '`cantidad_inv_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cantidad_inv_producto->Sortable = TRUE; // Allow sort
		$this->cantidad_inv_producto->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['cantidad_inv_producto'] = &$this->cantidad_inv_producto;

		// descripcion
		$this->descripcion = new DbField('inventario_producto', 'inventario_producto', 'x_descripcion', 'descripcion', '`descripcion`', '`descripcion`', 200, -1, FALSE, '`descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// estado
		$this->estado = new DbField('inventario_producto', 'inventario_producto', 'x_estado', 'estado', '`estado`', '`estado`', 16, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// precio
		$this->precio = new DbField('inventario_producto', 'inventario_producto', 'x_precio', 'precio', '`precio`', '`precio`', 4, -1, FALSE, '`precio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`inventario_producto`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('id_inventario', $rs))
				AddFilter($where, QuotedName('id_inventario', $this->Dbid) . '=' . QuotedValue($rs['id_inventario'], $this->id_inventario->DataType, $this->Dbid));
			if (array_key_exists('fecha_inventario', $rs))
				AddFilter($where, QuotedName('fecha_inventario', $this->Dbid) . '=' . QuotedValue($rs['fecha_inventario'], $this->fecha_inventario->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->cantidad_inv_producto->DbValue = $row['cantidad_inv_producto'];
	...
	{
		return "`id_producto` = '@id_producto@' AND `id_inventario` = '@id_inventario@' AND `fecha_inventario` = '@fecha_inventario@'";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('id_inventario', $row) ? $row['id_inventario'] : NULL) : $this->id_inventario->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_inventario@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		$val = is_array($row) ? (array_key_exists('fecha_inventario', $row) ? $row['fecha_inventario'] : NULL) : $this->fecha_inventario->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@fecha_inventario@", AdjustSql(UnFormatDateTime($val,0), $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "inventario_productolist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "inventario_productoedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "inventario_productoadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("inventario_productoview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("inventario_productoview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "inventario_productoadd.php?" . $this->getUrlParm($parm);
		else
			$url = "inventario_productoadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("inventario_productoedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("inventario_productoadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("inventario_productodelete.php", $this->getUrlParm());
	}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",id_inventario:" . JsonEncode($this->id_inventario->CurrentValue, "string");
		$json .= ",fecha_inventario:" . JsonEncode($this->fecha_inventario->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->id_inventario->CurrentValue != NULL) {
			$url .= "&id_inventario=" . urlencode($this->id_inventario->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		if ($this->fecha_inventario->CurrentValue != NULL) {
			$url .= "&fecha_inventario=" . urlencode($this->fecha_inventario->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_inventario") !== NULL)
				$arKey[] = Param("id_inventario");
			...
			elseif (IsApi() && Route(3) !== NULL)
				...
				$arKeys = NULL; // Do not setup
			if (Param("fecha_inventario") !== NULL)
				$arKey[] = Param("fecha_inventario");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->id_producto->CurrentValue = $key[0];
			$this->id_inventario->CurrentValue = $key[1];
			$this->fecha_inventario->CurrentValue = $key[2];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->cantidad_inv_producto->setDbValue($rs->fields('cantidad_inv_producto'));
	...
	public function renderEditRow()
	{
		...
		$this->fecha_inventario->EditValue = $this->fecha_inventario->CurrentValue;
		$this->fecha_inventario->EditValue = FormatDateTime($this->fecha_inventario->EditValue, 0);
		$this->fecha_inventario->ViewCustomAttributes = "";

		// cantidad_inv_producto
		$this->cantidad_inv_producto->EditAttrs["class"] = "form-control";
		$this->cantidad_inv_producto->EditCustomAttributes = "";
		$this->cantidad_inv_producto->EditValue = $this->cantidad_inv_producto->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->cantidad_inv_producto);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->cantidad_inv_producto);
