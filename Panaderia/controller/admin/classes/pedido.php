...
class pedido extends DbTable
{
	...
	public function __construct()
	{
		...
		// id_pedido
		$this->id_pedido = new DbField('pedido', 'pedido', 'x_id_pedido', 'id_pedido', '`id_pedido`', '`id_pedido`', 200, -1, FALSE, '`id_pedido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_pedido->IsPrimaryKey = TRUE; // Primary key field
		...
		// documento_cliente
		$this->documento_cliente = new DbField('pedido', 'pedido', 'x_documento_cliente', 'documento_cliente', '`documento_cliente`', '`documento_cliente`', 200, -1, FALSE, '`documento_cliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->documento_cliente->Sortable = TRUE; // Allow sort
		$this->fields['documento_cliente'] = &$this->documento_cliente;

		// id_producto
		$this->id_producto = new DbField('pedido', 'pedido', 'x_id_producto', 'id_producto', '`id_producto`', '`id_producto`', 200, -1, FALSE, '`id_producto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_producto->Sortable = TRUE; // Allow sort
		$this->fields['id_producto'] = &$this->id_producto;

		// id_inventario
		$this->id_inventario = new DbField('pedido', 'pedido', 'x_id_inventario', 'id_inventario', '`id_inventario`', '`id_inventario`', 200, -1, FALSE, '`id_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_inventario->Sortable = TRUE; // Allow sort
		$this->fields['id_inventario'] = &$this->id_inventario;

		// fecha_inventario
		$this->fecha_inventario = new DbField('pedido', 'pedido', 'x_fecha_inventario', 'fecha_inventario', '`fecha_inventario`', CastDateFieldForLike('`fecha_inventario`', 0, "DB"), 133, 0, FALSE, '`fecha_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// cantidad_pedido
		$this->cantidad_pedido = new DbField('pedido', 'pedido', 'x_cantidad_pedido', 'cantidad_pedido', '`cantidad_pedido`', '`cantidad_pedido`', 3, -1, FALSE, '`cantidad_pedido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cantidad_pedido->Sortable = TRUE; // Allow sort
		$this->cantidad_pedido->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['cantidad_pedido'] = &$this->cantidad_pedido;

		// precio_pedido
		$this->precio_pedido = new DbField('pedido', 'pedido', 'x_precio_pedido', 'precio_pedido', '`precio_pedido`', '`precio_pedido`', 3, -1, FALSE, '`precio_pedido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->precio_pedido->Sortable = TRUE; // Allow sort
		$this->precio_pedido->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['precio_pedido'] = &$this->precio_pedido;

		// tiempo_pedido
		$this->tiempo_pedido = new DbField('pedido', 'pedido', 'x_tiempo_pedido', 'tiempo_pedido', '`tiempo_pedido`', CastDateFieldForLike('`tiempo_pedido`', 4, "DB"), 134, 4, FALSE, '`tiempo_pedido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tiempo_pedido->Sortable = TRUE; // Allow sort
		$this->tiempo_pedido->DefaultErrorMessage = str_replace("%s", $GLOBALS["TIME_SEPARATOR"], $Language->phrase("IncorrectTime"));
		$this->fields['tiempo_pedido'] = &$this->tiempo_pedido;

		// estado
		$this->estado = new DbField('pedido', 'pedido', 'x_estado', 'estado', '`estado`', '`estado`', 16, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->estado->Sortable = TRUE; // Allow sort
		$this->estado->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['estado'] = &$this->estado;

		// total_pedido
		$this->total_pedido = new DbField('pedido', 'pedido', 'x_total_pedido', 'total_pedido', '`total_pedido`', '`total_pedido`', 3, -1, FALSE, '`total_pedido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->total_pedido->Sortable = TRUE; // Allow sort
		$this->total_pedido->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['total_pedido'] = &$this->total_pedido;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`pedido`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_pedido', $rs))
				AddFilter($where, QuotedName('id_pedido', $this->Dbid) . '=' . QuotedValue($rs['id_pedido'], $this->id_pedido->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->documento_cliente->DbValue = $row['documento_cliente'];
		...
		$this->cantidad_pedido->DbValue = $row['cantidad_pedido'];
		$this->precio_pedido->DbValue = $row['precio_pedido'];
		$this->tiempo_pedido->DbValue = $row['tiempo_pedido'];
		$this->estado->DbValue = $row['estado'];
		$this->total_pedido->DbValue = $row['total_pedido'];
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('id_pedido', $row) ? $row['id_pedido'] : NULL) : $this->id_pedido->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_pedido@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("pedidoview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		$json = "";
		$json .= "id_pedido:" . JsonEncode($this->id_pedido->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->id_pedido->CurrentValue != NULL) {
			$url .= "id_pedido=" . urlencode($this->id_pedido->CurrentValue);
		} else {
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->id_pedido->CurrentValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->documento_cliente->setDbValue($rs->fields('documento_cliente'));
		...
		$this->cantidad_pedido->setDbValue($rs->fields('cantidad_pedido'));
		$this->precio_pedido->setDbValue($rs->fields('precio_pedido'));
		$this->tiempo_pedido->setDbValue($rs->fields('tiempo_pedido'));
		$this->estado->setDbValue($rs->fields('estado'));
		$this->total_pedido->setDbValue($rs->fields('total_pedido'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->documento_cliente->EditValue = $this->documento_cliente->CurrentValue;
		...
		$this->cantidad_pedido->EditValue = $this->cantidad_pedido->CurrentValue;
		...
		$this->precio_pedido->EditValue = $this->precio_pedido->CurrentValue;
		...
		$this->tiempo_pedido->EditValue = $this->tiempo_pedido->CurrentValue;
		...
		$this->total_pedido->EditValue = $this->total_pedido->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->documento_cliente);
					$doc->exportCaption($this->id_producto);
					$doc->exportCaption($this->id_inventario);
					$doc->exportCaption($this->fecha_inventario);
					$doc->exportCaption($this->cantidad_pedido);
					$doc->exportCaption($this->precio_pedido);
					$doc->exportCaption($this->tiempo_pedido);
					$doc->exportCaption($this->estado);
					$doc->exportCaption($this->total_pedido);
				} else {
					$doc->exportCaption($this->id_pedido);
					$doc->exportCaption($this->documento_cliente);
					$doc->exportCaption($this->id_producto);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->documento_cliente);
						$doc->exportField($this->id_producto);
						$doc->exportField($this->id_inventario);
						$doc->exportField($this->fecha_inventario);
						$doc->exportField($this->cantidad_pedido);
						$doc->exportField($this->precio_pedido);
						$doc->exportField($this->tiempo_pedido);
						$doc->exportField($this->estado);
						$doc->exportField($this->total_pedido);
					} else {
						$doc->exportField($this->id_pedido);
						$doc->exportField($this->documento_cliente);
						$doc->exportField($this->id_producto);
