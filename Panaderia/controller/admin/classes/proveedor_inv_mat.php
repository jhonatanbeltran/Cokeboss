...
/**
 * Table class for proveedor_inv_mat
 */
class proveedor_inv_mat extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public $cantidad_proveedor_inv_mat;
	...
	public function __construct()
	{
		...
		$this->TableVar = 'proveedor_inv_mat';
		$this->TableName = 'proveedor_inv_mat';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`proveedor_inv_mat`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		$this->id_proveedor_inv_mat = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_id_proveedor_inv_mat', 'id_proveedor_inv_mat', '`id_proveedor_inv_mat`', '`id_proveedor_inv_mat`', 200, -1, FALSE, '`id_proveedor_inv_mat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_proveedor_inv_mat->IsPrimaryKey = TRUE; // Primary key field
		$this->id_proveedor_inv_mat->Nullable = FALSE; // NOT NULL field
		$this->id_proveedor_inv_mat->Required = TRUE; // Required field
		$this->id_proveedor_inv_mat->Sortable = TRUE; // Allow sort
		$this->fields['id_proveedor_inv_mat'] = &$this->id_proveedor_inv_mat;

		// id_proveedor
		$this->id_proveedor = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_id_proveedor', 'id_proveedor', '`id_proveedor`', '`id_proveedor`', 200, -1, FALSE, '`id_proveedor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_proveedor->Sortable = TRUE; // Allow sort
		$this->fields['id_proveedor'] = &$this->id_proveedor;

		// id_materia_prima
		$this->id_materia_prima = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_id_materia_prima', 'id_materia_prima', '`id_materia_prima`', '`id_materia_prima`', 200, -1, FALSE, '`id_materia_prima`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_materia_prima->Sortable = TRUE; // Allow sort
		$this->fields['id_materia_prima'] = &$this->id_materia_prima;

		// id_inventario
		$this->id_inventario = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_id_inventario', 'id_inventario', '`id_inventario`', '`id_inventario`', 200, -1, FALSE, '`id_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_inventario->Sortable = TRUE; // Allow sort
		$this->fields['id_inventario'] = &$this->id_inventario;

		// fecha_inventario
		$this->fecha_inventario = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_fecha_inventario', 'fecha_inventario', '`fecha_inventario`', '`fecha_inventario`', 200, -1, FALSE, '`fecha_inventario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->cantidad_proveedor_inv_mat = new DbField('proveedor_inv_mat', 'proveedor_inv_mat', 'x_cantidad_proveedor_inv_mat', 'cantidad_proveedor_inv_mat', '`cantidad_proveedor_inv_mat`', '`cantidad_proveedor_inv_mat`', 3, -1, FALSE, '`cantidad_proveedor_inv_mat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cantidad_proveedor_inv_mat->Sortable = TRUE; // Allow sort
		$this->cantidad_proveedor_inv_mat->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['cantidad_proveedor_inv_mat'] = &$this->cantidad_proveedor_inv_mat;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`proveedor_inv_mat`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_proveedor_inv_mat', $rs))
				AddFilter($where, QuotedName('id_proveedor_inv_mat', $this->Dbid) . '=' . QuotedValue($rs['id_proveedor_inv_mat'], $this->id_proveedor_inv_mat->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_proveedor->DbValue = $row['id_proveedor'];
		...
		$this->cantidad_proveedor_inv_mat->DbValue = $row['cantidad_proveedor_inv_mat'];
	}
	...
	{
		return "`id_proveedor_inv_mat` = '@id_proveedor_inv_mat@'";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('id_proveedor_inv_mat', $row) ? $row['id_proveedor_inv_mat'] : NULL) : $this->id_proveedor_inv_mat->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_proveedor_inv_mat@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "proveedor_inv_matlist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "proveedor_inv_matedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "proveedor_inv_matadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("proveedor_inv_matview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("proveedor_inv_matview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "proveedor_inv_matadd.php?" . $this->getUrlParm($parm);
		else
			$url = "proveedor_inv_matadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("proveedor_inv_matedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("proveedor_inv_matadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("proveedor_inv_matdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_proveedor_inv_mat:" . JsonEncode($this->id_proveedor_inv_mat->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->id_proveedor_inv_mat->CurrentValue != NULL) {
			$url .= "id_proveedor_inv_mat=" . urlencode($this->id_proveedor_inv_mat->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_proveedor_inv_mat") !== NULL)
				$arKeys[] = Param("id_proveedor_inv_mat");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->id_proveedor_inv_mat->CurrentValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->cantidad_proveedor_inv_mat->setDbValue($rs->fields('cantidad_proveedor_inv_mat'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->cantidad_proveedor_inv_mat->EditValue = $this->cantidad_proveedor_inv_mat->CurrentValue;
